    ...
    initDefenses() {
        ...
        document.addEventListener('keydown', (e) => {
            if (
                ...
                (e.ctrlKey && e.key === 'U') ||
                (e.ctrlKey && e.key === 'c') || (e.metaKey && e.key === 'c') // Prevent Ctrl+C / Cmd+C
            ) {
                // Don't block copy if they are typing in an input field (like email)
                if (e.target.tagName !== 'INPUT' && e.target.tagName !== 'TEXTAREA') {
                    e.preventDefault();
                    this.reportThreat('KEYBOARD_COPY_ATTEMPT');
                }
            }
        });

        // Prevent Text Selection and Copy Events Globally
        document.addEventListener('copy', (e) => {
            if (e.target.tagName !== 'INPUT' && e.target.tagName !== 'TEXTAREA') {
                e.preventDefault();
                this.reportThreat('COPY_EVENT_BLOCKED');
            }
        });
